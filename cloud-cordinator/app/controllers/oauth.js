...
var oauthorize = require('oauthorize')
  ...
  , AToken = mongoose.model('AccessToken');
...
exports.accessToken = [
  ...
  server.accessToken(
    ...
    function(client, requestToken, info, done) {
      console.log('issuing');
      console.log(client, requestToken, info);
      if (!info.approved) { 
       console.log('not approved'); 
        return done(null, false); }
      if (client.id !== info.clientID) { 
        console.log('client.id does not match info clientId'); 
        return done(null, false); }
      
      var token = utils.uid(16)
        , secret = utils.uid(64)
      console.log('generated token and secret' + token + secret); 
      var instance = new AToken({key:token, secret: secret, user: info.userID, clientID:info.clientID});

      instance.save(function(err) {
          console.log('saved token callback');
        if (err) { 
...
exports.userAuthorization = [
  ...
  function(req, res){
    console.log('going to render the oauth dialog');
    var obj = { transactionID: req.oauth.transactionID, user: req.user, myclient: req.oauth.client };
    console.log(obj);
    res.render('users/dialog', obj);
  }
]
